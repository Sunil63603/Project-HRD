...
//why is this 'JSX.Element'? why not React.FC?
function MessageStudent(): JSX.Element {
  ...
  }, [studentUSN as string]); //empty dependency[] ensures that this runs only on initial render,but setInterval() is resposible for calling fetch() again and again
  ...
  const fetchConversationsWithStudent = async (): Promise<void> => {
    try {
      ...
      const data: registeredStuds[] = await response.json(); // Entire JSONBin record
