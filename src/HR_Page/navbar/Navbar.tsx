...
const Navbar: React.FC = () => {
  const [activeLink, setActiveLink] = useState<string>("/hr/create-job");
  const location = useLocation();

  //set the active link based on the current path
  useEffect(() => {
    setActiveLink(location.pathname);
  }, [location.pathname as string]); //TS for custom dependency array
      ...
      <ul className="flex gap-14 list-none p-0 m-0">
        ...
        >
          <Link
            ...
            className="text-white no-underline text-base p-2 rounded-md flex justify-center items-center transition-all"
            onClick={(e: React.MouseEvent<HTMLAnchorElement>) =>
              setActiveLink("/hr/companies-list")
            }
          >
            Visiting Companies
          </Link>
        ...
        >
          <Link
            ...
            className="text-white no-underline text-base p-2 rounded-md flex justify-center items-center transition-all"
            onClick={(e: React.MouseEvent<HTMLAnchorElement>) =>
              setActiveLink("/hr/message-container")
            }
          >
            Messages
          </Link>
        ...
        >
          <Link
            ...
            className="text-white no-underline text-base p-2 rounded-md flex justify-center items-center transition-all"
            onClick={(e: React.MouseEvent<HTMLAnchorElement>) =>
              setActiveLink("/hr/profile-container")
            }
          >
            Profiles
          </Link>
